...
public partial class PlayerBodyControllerMechanic : EntityComponent<Player>
{
    /// <summary>
    ...
    protected PlayerBodyController Controller => Entity.Components.Get<PlayerBodyController>();
    ...
    public PlayerBodyControllerMechanic()
    {
    ...
    /// <returns></returns>
    public bool TrySimulate(PlayerBodyController controller)
    {
        var before = IsActive;
    ...
    {
        var trace = Controller.TraceBBox(
            Controller.Position,
            Controller.Position + direction * 32f
        );
        if (!trace.Hit)
            return default;
    ...
    private static float ApproximateWallHeight(
        Vector3 startPos,
        Vector3 wallNormal,
        float maxHeight,
        float maxDist,
        int precision,
        out Vector3 tracePos,
        out float absoluteHeight
    )
    {
        ...
        for (int i = 0; i < Math.Min(precision, MaxWallTraceIterations); i++)
        {
            startPos.z += step;
            currentHeight += step;
            var trace = Trace.Ray(startPos, startPos - wallNormal * maxDist).Run();

            if (PlayerBodyController.Debug)
                DebugOverlay.TraceResult(trace);

            if (!trace.Hit && !foundWall)
                continue;
            if (trace.Hit)
            {
